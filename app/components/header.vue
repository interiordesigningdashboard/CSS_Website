<template>
  <nav class="fixed top-0 w-full bg-white/95 backdrop-blur-sm z-50 py-2.5 shadow-lg transition-all duration-300" 
    ...
    <div class="mx-auto px-8 md:px-4 flex justify-between items-center">
      <!-- Logo -->
      <div>
        <NuxtLink to="/" class="text-3xl md:text-2xl font-bold bg-gradient-to-r from-blue-600 to-orange-500 bg-clip-text text-transparent no-underline">
          <img src="/logo.jpg" alt="ColorsLogo" class="inline h-12" />
          <span class="hidden md:inline good-times pl-2">Colors & Signage Solutions</span>
        </NuxtLink>
      </div>
      
      <!-- Desktop Navigation Menu -->
      <ul class="list-none m-0 p-0 transition-all duration-300 ease-in-out"
        ...
        <li v-for="item in navigationItems" :key="item.name" class="w-full md:w-auto">
          <a 
            :href="item.href" 
